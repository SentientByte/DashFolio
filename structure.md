...
- **EWMA mean** (used for drift in simulations):
  ...
  \]
- **EWMA volatility**: \( \sigma_t = \sqrt{\frac{\sum_{i=0}^{n-1} w_i (r_{t-i} - \mu_t)^2}{\sum_{i=0}^{n-1} w_i}} \).
...
- **Potential loss** at stop `L`: \( \text{Loss} = (P_t - P_t (1 - L/100)) \times Q \)
  with `Q` representing the position size.
- **EWMA Value-at-Risk**: for confidence level `\alpha`,
  \( \text{VaR} = z_{1-\alpha} \sigma_t P_t Q \), where `z` is the standard
  normal quantile estimated in `Calculations/risk_analysis.py` via
  `numpy.percentile`.
